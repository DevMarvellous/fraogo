        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-16">
            ...
            <div className="bg-white rounded-2xl overflow-hidden shadow-card border border-border card-hover">
              <div className="p-8" style={{ background: 'linear-gradient(135deg, #0A4D2E, #1A6B42)' }}>
                ...
                <ul className="space-y-2.5 mb-6">
                  {[
                    ...
                    <li key={benefit} className="flex items-start gap-2.5 text-sm text-muted-foreground">
                      <Star className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: '#C9A84C' }} />
                      {benefit}
                    </li>
                  ))}
                </ul>
            ...
            {/* Hire a Vendor */}
            <div className="bg-white rounded-2xl overflow-hidden shadow-card border border-border card-hover">
              <div className="p-8" style={{ background: 'linear-gradient(135deg, #92400e, #b45309)' }}>
                <div className="w-14 h-14 rounded-2xl bg-white/15 flex items-center justify-center mb-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            {[
              ...
              <Link
                key={cat.label}
                href="/general-service/rental/hire-vendor"
                className="flex flex-col items-center gap-2 p-4 bg-white rounded-xl border border-border hover:border-[#0A4D2E] hover:bg-green-50 transition-all text-center group"
              >
                <span className="text-2xl">{cat.emoji}</span>
                <span className="text-xs font-semibold text-muted-foreground group-hover:text-[#0A4D2E] transition-colors">{cat.label}</span>
              </Link>
            ))}
          </div>
        </div>
